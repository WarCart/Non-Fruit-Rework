...
public class CSpawnPacifistaModelPacket implements IPacket<CSpawnPacifistaModelPacket> {
    private PacifistaModel model;

    public CSpawnPacifistaModelPacket(PacifistaModel model) {
        this.model = model;
    }

    public CSpawnPacifistaModelPacket() {}

    public void encode(PacketBuffer buffer) {
        buffer.writeEnum(model);
    }

    public CSpawnPacifistaModelPacket decode(PacketBuffer buffer) {
        CSpawnPacifistaModelPacket packet = new CSpawnPacifistaModelPacket();
        packet.model = buffer.readEnum(PacifistaModel.class);
        return packet;
    }
    ...
    public void handle(Supplier<NetworkEvent.Context> ctx) {
        if (ctx.get().getDirection() == NetworkDirection.PLAY_TO_SERVER) {
            ctx.get().enqueueWork(() -> {
                ...
                PacifistaEntity spawnedPacifista = ModEntities.PACIFISTA.get().spawn(player.getLevel(), null, null, null, player.blockPosition(), SpawnReason.MOB_SUMMONED, false, false);
                ...
                this.model.modify(spawnedPacifista);
