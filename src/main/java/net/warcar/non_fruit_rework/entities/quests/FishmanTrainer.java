...
public class FishmanTrainer extends TrainerEntity implements IHakiTrainer {
    ...
    public FishmanTrainer(EntityType type, World world) {
        super(type, world, EntityHelper.getTexture("hack"));
        if (!world.isClientSide) {
            ...
            //Fishman karate
            this.goalSelector.addGoal(2, new KachiageHaisokuWrapperGoal(this));
            this.goalSelector.addGoal(2, new UchimizuWrapperGoal(this));
            this.goalSelector.addGoal(2, new SamehadaShoteiWrapperGoal(this));
            if (this.getRandom().nextFloat() < 0.7) {
                this.goalSelector.addGoal(2, new MurasameWrapperGoal(this));
            }
            if (this.getRandom().nextFloat() < 0.4) {
                this.goalSelector.addGoal(2, new KarakusagawaraSeikenWrapperGoal(this));
            }
            //Haki
    ...
    public static AttributeModifierMap.MutableAttribute createAttributes() {
        return OPEntity.createAttributes().add(Attributes.FOLLOW_RANGE, 60.0D).add(Attributes.MOVEMENT_SPEED, 0.3).add(Attributes.ATTACK_DAMAGE, 10.0D).add(Attributes.MAX_HEALTH, 200.0D).add(Attributes.ARMOR, 20.0D).add(ForgeMod.SWIM_SPEED.get(), 5);
    }
