...
public class NekomamushiBoss extends OPBossEntity<NekomamushiBoss> {
    public static final EntityType<NekomamushiBoss> INSTANCE = WyRegistry.createEntityType(NekomamushiBoss::new).build("");

    private final NPCPhase<NekomamushiBoss> basicPhase = new SimplePhase<>("Basic phase", this);
    private final NPCPhase<NekomamushiBoss> sulongPhase = new SimplePhase<>("Sulong phase", this);
    private final NPCPhase<NekomamushiBoss> finalPhase = new SimplePhase<>("Final phase", this, NekomamushiBoss::startFinalPhase);
    ...
    public static AttributeModifierMap.MutableAttribute createAttributes() {
        return OPEntity.createAttributes().add(Attributes.FOLLOW_RANGE, 60.0).add(Attributes.MOVEMENT_SPEED, 0.3).add(Attributes.ATTACK_DAMAGE, 6.0).add(Attributes.MAX_HEALTH, 400.0).add(Attributes.ARMOR, 3.0).add(Attributes.KNOCKBACK_RESISTANCE, 0.2);
    }

    private static void startFinalPhase(NekomamushiBoss entity) {
        entity.setItemSlot(EquipmentSlotType.MAINHAND, new ItemStack(ModWeapons.BISENTO.get()));
    }
